...
// #private <dinput.h> <-- mingw does not like this header.
...
int libenjoy_get_joy_name(int index, const char *szRegKey, char *res) {
  ...
  if (regresult == ERROR_SUCCESS) {
    if (regsize >= 256) return -1;

    regresult = RegQueryValueExA(hKey, REGSTR_VAL_JOYOEMNAME, 0, 0, (LPBYTE)res, &regsize);
  }
  RegCloseKey(hKey);
  return 0;
}
...
void libenjoy_enumerate(libenjoy_context *ctx) {
  ...
  for (i = JOYSTICKID1; i < maxdevs && numdevs < LIBENJOY_MAX_JOYSTICK; ++i) {
    ...
    if (result == JOYERR_NOERROR) {
      ...
      if (!inf) inf = libenjoy_add_known_dev(ctx->os, guid, i);
  ...
  for (i = 0; existing_ids && i < existing_count; ++i) {
    if (existing_ids[i] == UINT_MAX) continue;
...
uint32_t *libenjoy_create_existing_ids(libenjoy_context *ctx) {
  ...
  if (ctx->info_list.list == 0) return res;

  res = (uint32_t *)calloc(ctx->info_list.count, sizeof(uint32_t));
  for (i = 0; i < ctx->info_list.count; ++i) res[i] = ctx->info_list.list[i]->id;
  return res;
}
...
libenjoy_os_specific *libenjoy_open_os_specific(libenjoy_context *ctx, uint32_t id) {
  ...
  if (result != JOYERR_NOERROR) return NULL;
  ...
  for (i = 0; i < BUTTONS_COUNT; ++i) os->buttons[i] = 0;
  return os;
}
...
void libenjoy_set_parts_count(libenjoy_joystick *joy) {
  ...
  result = joyGetDevCaps(inf->sys_id, &joycaps, sizeof(joycaps));
  if (result != JOYERR_NOERROR) goto failed;

  joy->num_buttons = joycaps.wNumButtons;
  joy->num_axes = joycaps.wNumAxes;

  if (joycaps.wCaps & JOYCAPS_HASPOV) joy->num_axes += 2;
...
void libenjoy_close_os_specific(libenjoy_os_specific *os) { free(os); }

void libenjoy_poll_priv(libenjoy_context *ctx) {
  MMRESULT result;
  DWORD pos[AXES_COUNT];
  const DWORD flags[AXES_COUNT] = {JOY_RETURNX, JOY_RETURNY, JOY_RETURNZ, JOY_RETURNR, JOY_RETURNU, JOY_RETURNV};
  ...
  for (j = 0; j < jlist->count; ++j) {
    ...
    for (i = 0; i < (joy->num_axes - HATS_COUNT); ++i) {
      if (!(joyinfo.dwFlags & flags[i])) continue;

      value = (int)(((float)pos[i] + os->axes_offset[i]) * os->axes_scale[i]);

      change = (value - os->axes[i]);
      if ((change > -JOY_AXIS_THRESHOLD) && (change < JOY_AXIS_THRESHOLD)) continue;
